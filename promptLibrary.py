...
from promptLibrary_preview import (
    DeleteRefToMissingImages,
    PreviewExlusivity,
    PreviewFiles,
    PreviewList,
    SetCachedPerviewFileDirty,
    SyncPreviewList,
    timer,
)
...
class CategoryList:
    ...
    def __init__(self, root, data, cat, onselect):
        ...
        self.dis = ttk.Checkbutton(
            self.frame,
            text="Disable",
            variable=self.disable,
            onvalue=True,
            command=self.cb_disabled,
        )
        self.always = ttk.Checkbutton(
            self.frame, text="Don't Ignore", variable=self.noIgnore, onvalue=True
        )
        self.lbox.configure(yscrollcommand=scrl.set)
        self.lbox.bind("<<ListboxSelect>>", lambda e: self.cb_change())
        self.lbox.bind(
            "<Double-Button-1>",
            lambda e: self.cb_edit(self.lbox.get(self.lbox.curselection())),
        )
        self.lbox.bind(
            "<Button-3>",
            lambda e: self.cb_copy(self.lbox.get(self.lbox.curselection())),
        )
        sep = ttk.Separator(self.frame, orient=HORIZONTAL)
        self.weight = ttk.Spinbox(
            self.frame,
            format="%.1f",
            increment=0.1,
            from_=0.0,
            to=10.0,
            textvariable=self.weightVal,
            width=5,
        ...
        btnDel.bind(
            "<Button-1>",
            lambda e: self.cb_delete(self.lbox.get(self.lbox.curselection())),
        )
    ...
    def cb_edit(self, prompt):
        ...
        if isValid:
            ...
            self.dat[self.cat] = {
                newPrompt if k == prompt and newPrompt != prompt else k: v
                for k, v in self.dat[self.cat].items()
            }
    ...
    def cb_delete(self, prompt):
        answer = messagebox.askyesno(
            ...
            message="Are you sure that you want to delete '{0}'?\n\n{1}".format(
                prompt, self.dat[self.cat][prompt]["Prompt"]
            ),
        )
        if answer:
...
class SettingsList(CategoryList):
    ...
    def cb_edit(self, setting):
        ...
        if isValid:
            ...
            self.dat[self.cat] = {
                newSetting if k == setting and newSetting != setting else k: v
                for k, v in self.dat[self.cat].items()
            }
            self.relist(self.dat)
            self.lbox.selection_set(idx)
    ...
    def cb_delete(self, prompt):
        answer = messagebox.askyesno(
            title="Confirmation",
            message="Are you sure that you want to delete '{0}'?\n\n{1}".format(
                prompt, self.dat[self.cat][prompt]["Setting"]
            ),
        )
        if answer:
            self.dat[self.cat].pop(prompt)
...
class PromptEdit:
    def __init__(self, root):
        ...
        ttk.Label(self.dlg, text="Neg.Prompt:").grid(
            row=4, columnspan=2, sticky=(N, S, W)
        )
...
class SettingsEdit:
    def __init__(self, root):
        ...
        ttk.Label(self.dlg, text="Settings:").grid(
            row=2, columnspan=2, sticky=(N, S, W)
        )
...
class PromptPreview:
    def __init__(self, root, cb_copyWith):
        ...
        self.cpyWBtn = ttk.Button(
            self.frame, text="Copy with Preview Parameters", command=self.copyWith
        )
        self.autoCpy = ttk.Checkbutton(
            self.frame, text="Auto Copy", variable=self.acpy, onvalue=True
        )
    ...
    def markText(self, searchText):
        ...
        if index != "":
            self.promptText.tag_add(
                "marked", index, "%s + %sc" % (index, countVar.get())
            )
...
class ImagePreview:
    ...
    def __init__(self, root, cb_del, cb_sel):
        ...
        self.selBtn = ttk.Button(
            btnFrame, text="Select", command=self.SelectImagePrompts
        )
        ...
        self.noCpyCkp = ttk.Checkbutton(
            btnFrame, text="Copy Model/Batch", variable=self.noCpy, onvalue=True
        )
    ...
    def UpdateVisRefLabel(self):
        ...
        self.lbl.config(
            text=f"Visual Reference - {self.imgIdx}/{len(self.images)} - Additional Prompts: {addStylesCount} {addStyles}"
        )
    ...
    def CopyImageSettings(self):
        ...
        if "parameters" in info:
            ...
            if not cpyModel:
                s = re.sub(
                    "Model hash: [a-zA-Z0-9]*[\s,]*|Batch size: [a-zA-Z0-9]*[\s,]*|Batch pos: [a-zA-Z0-9]*[\s,]*",
                    "",
                    s,
                )
...
class GridPreview:
    ...
    def cb_showImage(self, event):
        ...
        self.gridImageThread = threading.Thread(
            target=self.ShowFullSizeImage, args=(self.xlabel, self.ylabel)
        )
    ...
    def NextSubImage(self, event):
        if self.hasImage == False:
            return
        self.subImgIdx = (
            self.subImgIdx + 1 if self.subImgIdx + 1 <= self.subImgCnt else 1
        )
    ...
    def PreviousSubImage(self, event):
        if self.hasImage == False:
            return
        self.subImgIdx = (
            self.subImgIdx - 1 if self.subImgIdx - 1 > 0 else self.subImgCnt
        )
        self.SetImage(self.comboList[self.imgIdx - 1], self.subImgIdx - 1)
        self.UpdateVisRefLabel()
    ...
    def SetImage(self, combo, idx=0):
        xLabel = combo[0]
        yLabel = combo[1]
        fImg, flipped, gridCount = self.gridPreview(
            self.selection, xLabel, yLabel, cfIdx=idx
        )
    ...
    def ShowFullSizeImage(self, xLabel, yLabel):
        fSizeImg, *_ = self.gridPreview(
            self.selection, xLabel, yLabel, True, cfIdx=self.subImgIdx - 1
        )
        fSizeImg.show()
    ...
    def gridPreview(self, selection: dict, cat1="", cat2="", fullsize=False, cfIdx=0):
        ...
        if not fullsize:
            wf, hf = self._getSize(
                self.canvas.winfo_width() - padding[0],
                self.canvas.winfo_height() - padding[1],
                imgSize[0],
                imgSize[1],
            )
            ws = wf / imgSize[0]
        ...
        for ix, x in enumerate(xy):
            for iy, y in enumerate(x):
                ...
                if not fullsize:
                    ...
                    img = img.resize(size)
                if y[0] > 0:
                    imgExclBase = Image.new(
                        "RGBA",
                        (textsize_s * 2, textsize_s * 2),
                        color=(255, 255, 255, 0),
                    )
                    imgExcl = ImageDraw.Draw(imgExclBase)
                    imgExcl.polygon(
                        [(0, 0), (0, textsize_s * 2), (textsize_s * 2, 0)],
...
class Set:
    ...
    def __init__(self, root, name):
        ...
        self.iPreview = ImagePreview(
            ppFrame, self.cb_imageDeleted, self.cb_imageSelectPrompts
        )
        self.gPreview = GridPreview(
            ppFrame, self.catList, self.path, self.getPreviewFilesFromSelection
        )
        ...
        ttk.Separator(self.frame, orient=VERTICAL).grid(
            column=1, row=0, rowspan=len(struct) + 2, sticky=(N, W, E, S)
        )
        ...
        self.resetBtn = ttk.Button(
            btnFrame, text="Reset Selection", command=self.cb_reset
        )
        self.resetBtn.grid(column=0, row=0)

        self.gridBtn = ttk.Button(
            btnFrame, text="Toggle Grid View", command=self.cb_toggleGridView
        )
    ...
    def getPreviewFilesFromSelection(self, selection):
        ...
        commonFilesExcl, addStyles = PreviewExlusivity(
            selection, self.path, commonFiles
        )
    ...
    def createPreviewList(self, missing):
        ...
        if not promptList or not promptList[0].Settings:
            messagebox.showinfo(
                "Prompt File created", f"Prompt File for {pl} previews created!"
            )
        else:
            messagebox.showinfo(
                "Prompt File created",
                f"Prompt File for {pl} previews created using {sl} setting(s)!",
            )
...
class SetEdit:
    ...
    def __init__(self, root, name):
        ...
        self.updateBtn = ttk.Button(
            btnFrame, text="Update Selected", command=self.cb_update
        )
        self.updateBtn.grid(row=0, column=1)

        self.removeBtn = ttk.Button(
            btnFrame, text="Remove Selected", command=self.cb_remove
        )
    ...
    def updateList(self):
        ...
        for idx, cat in enumerate(self.struct):
            self.setContent.insert(
                parent="", index="end", iid=idx, text="", values=(cat, idx)
            )
    ...
    def cb_insert(self):
        ...
        if catName in self.struct:
            messagebox.showerror(
                "Item already exists",
                "Can't add an item that already exists",
                parent=self.tl,
            )
            return
        else:
    ...
    def cb_update(self):
        ...
        if self.setContent.focus() == "":
            messagebox.showerror(
                "No item selected",
                "You have to select an Item before you can update",
                parent=self.tl,
            )
            return
        ...
        if catName in self.struct and catName != curItem:
            messagebox.showerror(
                "Item already exists",
                "Can't rename an item to an already existing item",
                parent=self.tl,
            )
        else:
            self.struct = {
                catName if k == curItem else k: v for k, v in self.struct.items()
            }
    ...
    def cb_remove(self):
        if self.setContent.focus() == "":
            messagebox.showerror(
                "No item selected",
                "You have to select an Item you want to remove",
                parent=self.tl,
            )
            return
    ...
    def cb_save(self):
        ...
        if self.path == "":
            ...
            if os.path.exists(self.path):
                messagebox.showerror(
                    "Save Error", "A set with this name already exists!", parent=self.tl
                )
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Save Error", "Failed to create folder!", parent=self.tl
                )
                return
        ...
        if self.path != self.setName.get():
            try:
                ...
                messagebox.showerror(
                    "Save Error", "Failed to rename set!", parent=self.tl
                )
                return
...
def main():
    ...
    def on_edit():
        ...
        if sets[pset].dirty:
            messagebox.showerror(
                "There are unsafed changes",
                "Changes have to be saved before a set can be edited",
            )
        else:
    ...
    def on_copy():
        ...
        if newSet:
            if os.path.exists(newSet):
                messagebox.showerror(
                    "Save Error", "A set with this name already exists!", parent=root
                )
                return
            try:
                os.mkdir(newSet)
                ...
                messagebox.showerror(
                    "Save Error", "Failed to create folder!", parent=root
                )
                return
    ...
    def on_createPreviewList(missing):
        ...
        if sets[pset].dirty:
            messagebox.showerror(
                "There are unsafed changes",
                "Changes have to be saved before list is generated",
            )
            return
    ...
    previewmenu.add_command(
        label="Create list of missing previews (from selection)",
        command=on_createPreviewListMissing,
    )
    previewmenu.add_command(
        label="Create list of all previews (from selection)",
        command=on_createPreviewListAll,
    )
